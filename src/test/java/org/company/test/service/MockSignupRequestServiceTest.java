...
public class MockSignupRequestServiceTest {
    ...
    public static JavaArchive createTestArchive() {
        return ShrinkWrap.create(JavaArchive.class, "test.jar").addClass(SignupRequest.class).addClass(Status.class).addClass(SignupRequestService.class).addClass(MockSignupRequestService.class).addClass(Predicate.class).addClass(SignupRequestNotFoundException.class).addPackages(false, Confirmed.class.getPackage()).addAsManifestResource(EmptyAsset.INSTANCE, "beans.xml");
    }
    @Inject
    ...
    public void testSignupService() {
        ...
        SignupRequest _unconfirmedRequest = service.getAllUnconfirmedRequests().get(0);
        ...
        SignupRequest _confirmedRequest = service.getAllConfirmedRequests().get(0);

        assertNotNull(_confirmedRequest);

        service.approve(_confirmedRequest.getId());

        SignupRequest _approvedRequest = service.getAllApprovedRequests().get(0);
