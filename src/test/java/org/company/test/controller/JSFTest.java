...
public class JSFTest {
    ...
    public static WebArchive createTestArchive() {
        WebArchive jar = ShrinkWrap.create(WebArchive.class, "test.war") // .addAsLibraries(
                // resolver.artifact(
                ...
                // .resolveAsFiles())
                .addClasses(SignupRequest.class, Status.class).addClass(LoggerProducer.class).addClass(Notifier.class).addClass(LogNotifier.class).addClass(SignupRequestService.class).addClass(MockSignupRequestService.class).addClass(SignupRequestListProducer.class).addClass(Predicate.class).addClass(SignupRequestNotFoundException.class).addClasses(Approved.class, Confirmed.class, Denied.class,
                Registered.class).addClass(LoggedIn.class).addPackage(LoginAction.class.getPackage()).addAsResource("users.properties", "users.properties").addAsResource("roles.properties", "roles.properties").addAsWebResource(new File(WEBAPP_SRC, "index.html")).addAsWebResource(new File(WEBAPP_SRC, "index.xhtml")).addAsWebResource(new File(WEBAPP_SRC, "login.xhtml")).addAsWebResource(new File(WEBAPP_SRC, "ok.xhtml")).addAsWebResource(new File(WEBAPP_SRC, "register.xhtml")).addAsWebResource(new File(WEBAPP_SRC, "401.xhtml")).addAsWebResource(new File(WEBAPP_SRC, "403.xhtml")).addAsWebResource(new File(WEBAPP_SRC, "404.xhtml")).addAsWebResource(new File(WEBAPP_SRC, "500.xhtml")).addAsWebResource(new File(WEBAPP_SRC, "admin/approved.xhtml"),
                "admin/approved.xhtml").addAsWebResource(
                new File(WEBAPP_SRC, "admin/confirmDenial.xhtml"),
                "admin/confirmDenial.xhtml").addAsWebResource(
                new File(WEBAPP_SRC, "admin/confirmed.xhtml"),
                "admin/confirmed.xhtml").addAsWebResource(new File(WEBAPP_SRC, "admin/denied.xhtml"),
                "admin/denied.xhtml").addAsWebResource(
                new File(WEBAPP_SRC, "admin/requestDetail.xhtml"),
                "admin/requestDetail.xhtml").addAsWebResource(
                new File(WEBAPP_SRC, "admin/unconfirmed.xhtml"),
                "admin/unconfirmed.xhtml").addAsWebResource(
                new File(WEBAPP_SRC,
                "resources/bscomp/formInputField.xhtml"),
                "resources/bscomp/formInputField.xhtml").addAsWebResource(
                new File(WEBAPP_SRC, "resources/css/bootstrap.css"),
                "resources/css/bootstrap.css").addAsWebResource(
                new File(WEBAPP_SRC, "resources/css/container-app.css"),
                "resources/css/container-app.css").addAsWebResource(
                new File(WEBAPP_SRC, "resources/css/screen.css"),
                "resources/css/screen.css").addAsWebResource(
                new File(WEBAPP_SRC, "resources/gfx/banner.png"),
                "resources/gfx/banner.png").addAsWebResource(
                new File(WEBAPP_SRC, "resources/gfx/logo.png"),
                "resources/gfx/logo.png").addAsWebResource(
                new File(WEBAPP_SRC, "resources/images/logo.png"),
                "resources/images/logo.png").addAsWebInfResource(
                new File(WEBINF_SRC, "templates/default.xhtml"),
                "templates/default.xhtml").addAsWebInfResource(
                new File(WEBINF_SRC, "templates/container-app.xhtml"),
                "templates/container-app.xhtml").addAsWebInfResource(
                new File(WEBINF_SRC, "templates/topbar.xhtml"),
                "templates/topbar.xhtml").addAsWebInfResource(new File(WEBINF_SRC, "faces-config.xml")).addAsWebInfResource(new File(WEBINF_SRC, "jboss-web.xml")).addAsWebInfResource(EmptyAsset.INSTANCE, "beans.xml").setWebXML(new File(WEBINF_SRC, "web.xml"));
    ...
    public void testLoginPage(JSFServerSession server, JSFClientSession client)
        ...
        Assert.assertEquals("user", server.getFacesContext().getExternalContext().getRemoteUser());
        log.info("server.getFacesContext().getExternalContext().getUserPrincipal()@"
                + server.getFacesContext().getExternalContext().getUserPrincipal());
        log.info("isUserInRoles:ROLE_VIEWER@"
                + server.getFacesContext().getExternalContext().isUserInRole("ROLE_VIEWER"));
        log.info("isUserInRoles:ROLE_ADMINISTRATOR@"
                + server.getFacesContext().getExternalContext().isUserInRole("ROLE_ADMINISTRATOR"));

        client.click("logoutForm:logoutButton");
        Assert.assertEquals("/login.xhtml", server.getCurrentViewID());
        Assert.assertNull(server.getFacesContext().getExternalContext().getRemoteUser());
    }
    ...
    public void testSignupProgress(JSFServerSession server,
        ...
        Assert.assertEquals("/admin/unconfirmed.xhtml",
                server.getCurrentViewID());
        Assert.assertEquals("admin", server.getFacesContext().getExternalContext().getRemoteUser());
        log.info("server.getFacesContext().getExternalContext().getUserPrincipal()@"
                + server.getFacesContext().getExternalContext().getUserPrincipal());
